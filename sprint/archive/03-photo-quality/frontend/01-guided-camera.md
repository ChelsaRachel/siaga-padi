...
- 2026-08-03 — Audit follow-up: the flow had exactly ONE entry point (`useCreateCase`, right after case creation), so a case abandoned mid-capture could never be resumed — `syncFromServer` was unreachable in practice. Added `pages/case-detail/parts/ResumePhotoCard.tsx`, rendered whenever the case status is still photo-pending (`features/case/photo/photo-status-rules.ts`, mirroring the backend's `ADVANCE_FROM_STATUSES`), plus `CaseDetailPage.test.tsx`. Hydration now also resumes at the result step on the slot still missing a photo, so prior verdicts and the escalate option are visible on re-entry.
