...
## SIAGA PADI — FRD MVP Web/PWA untuk Triase Visual Penyakit Padi dan Rekomendasi Berbasis Bukti

> **Document ID:** `FRD-SIPADI-WEB-001`  
> **Versi:** `0.2.0`  
...
>
> **Cakupan channel:** dokumen ini hanya menjadi baseline development dan UAT untuk **responsive web application/PWA**. Aplikasi native mobile tidak termasuk acceptance scope dokumen ini dan akan didetailkan secara terpisah dalam `FRD-SIPADI-MOBILE-001` ketika Phase 2 dimulai.
>
...
| Nama Objective / Modul | `Web/PWA MVP — Triase Visual Penyakit Padi dan AI Recommendation & Explanation Engine` |
| Document ID | `FRD-SIPADI-WEB-001` |
| Versi | `0.2.0` |
| Status | `Draft for Review` |
| Prioritas | `Must` |
...
| Dokumen Terkait | `UX flow Web/PWA, API Spec, Dataset Card, Model Card, Knowledge Base Catalog, UAT Plan, future FRD-SIPADI-MOBILE-001` |
...
| 0.2.0 | 2026-07-18 | Fahri & Chelsa | Scope, architecture, future phase, appendices | Memisahkan FRD Web/PWA dari future Mobile FRD; menghapus struktur folder dan template feature |
| 0.3.0 | TBD | Project Team | Setelah validasi domain | Penyesuaian kelas penyakit, aturan rekomendasi, dan target model |
| 1.0.0 | TBD | Project Team | Approved baseline | Baseline Web/PWA siap development/UAT pilot |
...
`Menyediakan triase visual awal penyakit daun padi melalui kamera smartphone, rekomendasi tindakan berbasis sumber tervalidasi, dan workflow review penyuluh melalui responsive web application/PWA.`
...
Fase pertama dan seluruh acceptance scope dalam dokumen ini berupa web application/PWA karena paling realistis untuk dua AI Engineer dengan laptop dan smartphone: tidak memerlukan distribusi aplikasi native, dapat menggunakan kamera dan GPS smartphone, serta lebih cepat diuji lintas perangkat. Aplikasi native Android merupakan produk Phase 2 yang memiliki FRD tersendiri; requirement mobile seperti local database, background sync, push notification, native permission lifecycle, dan app distribution tidak dianggap selesai melalui FRD Web/PWA ini.
...
- Responsive web app/PWA untuk browser smartphone dan desktop.
...
| Aplikasi native Android (`FRD-SIPADI-MOBILE-001`) | Phase 2 | Memerlukan requirement, UX, permission, distribution, dan UAT khusus mobile |
| Offline local database dan background synchronization | Phase 2 Mobile | Membutuhkan conflict resolution, retry policy, dan device-storage governance |
| On-device image-quality assessment | Phase 2 Mobile | Model ringan perlu diekspor dan diuji lintas perangkat |
| On-device disease inference | Phase 3 Mobile | Memerlukan compression, benchmark performa, dan validasi domain di perangkat |
...
## 3.5 Pemisahan FRD Web/PWA dan Mobile

| Aspek | `FRD-SIPADI-WEB-001` — Dokumen Ini | `FRD-SIPADI-MOBILE-001` — Future Phase 2 |
|---|---|---|
| Channel | Browser mobile/desktop dan installable PWA | Native Android application |
| Tujuan release | Memvalidasi problem, workflow, CV, recommendation engine, dan human review | Mengoptimalkan penggunaan lapangan setelah Web/PWA tervalidasi |
| UI dan navigasi | Responsive web UI, browser routing, service worker | Native navigation, lifecycle, app state, deep link |
| Kamera dan lokasi | Web Camera/File API dan browser geolocation | Native camera pipeline, native permission, richer capture metadata |
| Offline | PWA draft dan retry terbatas | Local database, offline-first queue, background sync, conflict resolution |
| Notifikasi | In-app; PWA push ditunda | Native push notification dan reminder |
| Distribusi | URL/deployment web | APK/internal distribution/Play Store strategy |
| Acceptance dan UAT | Browser/device matrix untuk Web/PWA | Android device/OS matrix dan mobile-specific UAT |
| Dokumen bersama | API contract, data contract, model card, dataset card, KB catalog, safety policies | Menggunakan artefak bersama yang sama dan menambah mobile-specific contracts |

**Aturan pemisahan:**

1. Fitur native mobile tidak boleh dimasukkan sebagai acceptance criterion atau Definition of Done MVP Web/PWA.
2. Backend, model CV, recommendation engine, dan knowledge base harus channel-agnostic agar dapat digunakan kembali oleh aplikasi mobile.
3. Business rules dan permission tidak boleh hanya berada di frontend web; backend tetap menjadi enforcement point.
4. Perubahan kontrak bersama wajib menggunakan versioning dan compatibility review terhadap Web/PWA serta future mobile client.
5. Development mobile dimulai setelah exit criteria Web/PWA tercapai atau setelah ada keputusan eksplisit melalui decision log.
...
## 7.5 Prinsip Arsitektur untuk Future Mobile Client

- Seluruh fungsi inti tersedia melalui versioned API; tidak ada business-critical processing yang hanya hidup di browser.
- API authentication, authorization, idempotency, pagination, upload, dan error contract harus dapat digunakan oleh Web/PWA maupun future Android client.
- Client-generated UUID didukung untuk kasus/draft yang perlu disinkronkan tanpa menciptakan duplikasi.
- Metadata `client_type`, `client_version`, dan `capture_method` disimpan untuk observability, tanpa mengumpulkan fingerprint perangkat yang tidak diperlukan.
- Web/PWA-specific code seperti service worker dan IndexedDB tidak menjadi dependency pada domain service atau AI service.
- Native mobile implementation, local database schema, background task, push notification, secure storage, dan app distribution berada di luar dokumen ini.

## 7.6 Keputusan Provider LLM untuk MVP
...
Viewport smartphone menjadi prioritas untuk capture dan hasil petani pada Web/PWA. Viewport desktop/tablet dioptimalkan untuk review, dashboard, dan admin. Pernyataan ini tidak mencakup aplikasi native mobile.
...
| Native Android app (`FRD-SIPADI-MOBILE-001`) | Phase 2 | Memerlukan FRD dan UAT terpisah setelah validasi Web/PWA |
...
- [ ] FRD Web/PWA v0.2 disepakati Fahri/Chelsa.
...
| OQ-016 | Exit criteria apa yang memicu penyusunan `FRD-SIPADI-MOBILE-001`? | Phase 2 sequencing | Fahri & Chelsa | Sebelum roadmap Phase 2 | Open |
...
| DEC-002 | 2026-07-17 | Web-first responsive PWA | Native/web/PWA | Cepat diuji dengan smartphone, biaya rendah | Architecture dan MVP scope |
...
| DEC-015 | 2026-07-18 | Web/PWA dan native mobile menggunakan FRD terpisah | Satu FRD gabungan/dua FRD | Mencegah scope dan acceptance criteria tercampur | Dokumen ini menjadi `FRD-SIPADI-WEB-001`; mobile memakai `FRD-SIPADI-MOBILE-001` |
| DEC-016 | 2026-07-18 | Mobile development dimulai setelah validasi Web/PWA atau keputusan eksplisit | Parallel sejak awal/sequential | Menjaga kapasitas dua-person team dan mengurangi rework | Release sequencing |
