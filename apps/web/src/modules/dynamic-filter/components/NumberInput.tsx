...
export function NumberInput({ id, filterKey, config, placeholder = '0', min, max, debounceMs = 400 }: NumberInputProps) {
  ...
  return <Input id={id} type="number" value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} min={min} max={max} className="w-50" />
}
