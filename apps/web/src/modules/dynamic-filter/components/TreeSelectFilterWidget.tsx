...
function TreeNodeItem({
  ...
  return (
    <div>
      <div
        className={cn(
          'flex items-center gap-1 rounded text-sm cursor-pointer hover:bg-muted py-1.5',
          selectedValue === node.value && 'bg-muted font-medium',
        )}
...
export function TreeSelectFilterWidget({
  id,
  filterKey,
  nodes,
  config,
  placeholder = 'Pilih...',
}: TreeSelectFilterWidgetProps) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-55 p-2" align="start">
        <ScrollArea className="max-h-64">
          {nodes.map((node) => (
            <TreeNodeItem key={node.value} node={node} onSelect={handleSelect} selectedValue={selected?.value} />
          ))}
        </ScrollArea>
        {selected && (
          <button onClick={() => { setSelected(null); clear() }} className="w-full mt-2 text-xs text-muted-foreground hover:text-destructive text-center">
            Hapus pilihan
          </button>
        )}
      </PopoverContent>
