...
interface SelectAsyncFilterWidgetProps {
  id: string
  filterKey: string
...
export function SelectAsyncFilterWidget({
  id,
  filterKey,
  config,
  placeholder = 'Cari...',
      ...
      <PopoverContent className="w-50 p-2" align="start">
        ...
        {loading ? (
          ...
          <ScrollArea className="max-h-48">
            {options.map((opt) => (
              <button key={opt.value} onClick={() => handleSelect(opt)} className="w-full text-left px-2 py-1.5 text-sm rounded hover:bg-muted">
                {opt.label}
              </button>
            ))}
          </ScrollArea>
        ...
        {selected && (
          <button onClick={handleClear} className="w-full mt-2 text-xs text-muted-foreground hover:text-destructive text-center">
            Hapus pilihan
          </button>
        )}
      </PopoverContent>
