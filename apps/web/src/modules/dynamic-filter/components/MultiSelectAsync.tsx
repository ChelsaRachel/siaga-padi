...
export function MultiSelectAsync({ id, filterKey, config, placeholder = 'Cari...', fetchOptions, debounceMs = 400, maxDisplay = 2 }: MultiSelectAsyncProps) {
      ...
      <PopoverContent className="w-55 p-2" align="start">
        ...
        {loading ? (
          ...
          <ScrollArea className="max-h-48">
            ...
            {!loading && options.length === 0 && <p className="text-center text-xs text-muted-foreground py-4">Tidak ada hasil</p>}
          </ScrollArea>
        )}
        {selected.length > 0 && (
          <button onClick={() => { setSelected([]); clear() }} className="w-full mt-2 text-xs text-muted-foreground hover:text-destructive text-center">Hapus semua</button>
        )}
      </PopoverContent>
    </Popover>
  )
