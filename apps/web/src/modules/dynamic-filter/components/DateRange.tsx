...
export function DateRange({ id, filterKeyFrom, filterKeyTo, config, placeholder = 'Pilih rentang tanggal', outputFormat = 'yyyy-MM-dd' }: DateRangeProps) {
  ...
  function handleSelect(selected: DateRangeType | undefined) {
    setRange(selected)
    if (selected?.from && selected?.to) {
      emitMultiple({ [filterKeyFrom]: format(selected.from, outputFormat), [filterKeyTo]: format(selected.to, outputFormat) })
      setOpen(false)
    } else if (!selected) {
      clear()
    }
